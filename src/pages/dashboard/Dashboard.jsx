...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    api.get("/banners").then(({data})=>setBanners(Array.isArray(data) ? data : [])).catch(()=>{});
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)] transition-colors duration-200 relative pb-16">
      ...
      {/* 4 Compact Stats Cards - BENTO STYLE (BRAND VIEW) */}
      <section className="max-w-6xl ml-0 mr-auto px-4 py-4 relative z-10 w-full">
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-2">
           {[
             ...
             { 
               ...
               chartData: [{v: 2},{v: 4},{v: 3},{v: 6},{v: 5},{v: 7},{v: 8}] 
             },
             { 
             ...
             { 
               ...
               chartData: [{v: 4.2},{v: 4.3},{v: 4.1},{v: 4.5},{v: 4.6},{v: 4.7},{v: 4.8}] 
             }
           ].map((s, i) => (
             <div key={i} className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-3xl pt-6 px-6 pb-0 shadow-xl relative overflow-hidden group min-h-[160px] flex flex-col justify-between hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
               ...
               <div className="relative z-10 flex flex-col items-start gap-1">
                 ...
                 <div className="text-3xl font-black text-[var(--text-primary)] tracking-tighter my-1 flex items-baseline">
                   <AnimatedNumber value={s.value} format={s.format} prefix={s.prefix} />
                   {i === 3 && <span className="text-xl ml-1">%</span>}
                 </div>
               ...
               <div className="absolute bottom-0 left-0 right-0 h-20 pointer-events-none opacity-80 mt-auto translate-y-1 group-hover:translate-y-0 transition-transform">
                 <ResponsiveContainer width="100%" height="100%">
                   <AreaChart data={s.chartData} margin={{ top: 0, left: 0, right: 0, bottom: 0 }}>
                     <defs>
                       <linearGradient id={`gradient-brand-${i}`} x1="0" y1="0" x2="0" y2="1">
      ...
      {showPostRequestModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-3xl p-6 sm:p-8 max-w-md w-full relative shadow-2xl">
            <button 
