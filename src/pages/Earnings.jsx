    ...
    <div className="w-full max-w-none px-4 sm:px-6 lg:px-8 py-8 transition-all duration-300">
      ...
      <div className="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-secondary)] text-sm">Track your balance, view history, and manage bank information.</p>
        </div>
        ...
        {(pendingPayout > 0 && bankDetails) && (
          <button 
            ...
            className={`px-6 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all w-full sm:w-auto ${withdrawalRequested ? 'bg-[var(--bg-elevated)] text-[var(--text-secondary)] cursor-not-allowed' : 'bg-[#10B981] hover:bg-[#059669] text-[var(--text-primary)] shadow-[0_0_20px_rgba(16,185,129,0.3)]'}`}
          >
            {withdrawalRequested ? (
               <>Payout Requested ✓</>
            ) : (
            ...
            kycObj?.status === "pending" ? (
              <div id="kyc-banner-pending" className="bg-yellow-500/10 border border-yellow-500/20 rounded-3xl p-5 sm:p-6 text-[#FBBF24] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-start gap-3.5">
                  ...
                  <div>
                    <h4 className="text-[var(--text-primary)] font-bold text-sm">KYC Documents Under Review</h4>
              ...
              <div id="kyc-banner-unverified" className="bg-[var(--violet)]/10 border border-[var(--violet)]/20 rounded-3xl p-6 text-[var(--violet)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5 relative overflow-hidden">
                ...
                <div className="flex items-start gap-4 group cursor-pointer">
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[var(--text-tertiary)] text-[10px] flex items-center gap-1"><Sparkles size={11} /> 2 mins verification</span>
                    </div>
                ...
                <button 
                  onClick={() => setShowKycModal(true)} 
                  className="px-6 py-3 bg-[var(--violet)] hover:bg-[var(--violet-hover)] text-white rounded-xl font-bold text-xs uppercase tracking-widest transition-all shadow-lg shadow-[#7C5CFF]/20 shrink-0 whitespace-nowrap w-full sm:w-auto text-center"
                >
          ...
          <div className="bg-[var(--bg-elevated)] backdrop-blur-md border border-[var(--border-default)] rounded-3xl p-6 flex flex-col h-full">
            ...
            <div className="flex-grow flex flex-col space-y-4">
               {transactions.length > 0 && transactions.filter(t => t.status==='paid' || t.status==='SUCCESS').slice(0,3).map(t => (
                 <div key={t.id} className="bg-[var(--bg-elevated)] border border-[var(--border-default)] p-4 rounded-2xl flex items-center justify-between hover:bg-[var(--bg-elevated)] transition-colors">
                   <div>
                     <p className="text-sm font-bold text-[var(--text-primary)] truncate max-w-[150px]">{t.campaign_title || 'Collab Payment'}</p>
                     <p className="text-xs text-[var(--text-secondary)]">{(t.net_amount||t.amount).toLocaleString('en-IN')} INR</p>
                   </div>
            ...
            <button onClick={() => toast("Full invoice history coming soon")} className="mt-4 w-full border border-[var(--border-default)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] py-3 rounded-xl font-bold text-sm bg-[var(--bg-elevated)]">
               View All Invoices
            </button>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[var(--border-default)] bg-[var(--bg-elevated)]">
                  <th className="p-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider font-mono whitespace-nowrap">Campaign Name</th>
                  <th className="p-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider font-mono whitespace-nowrap">Amount (₹)</th>
                  <th className="p-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider font-mono whitespace-nowrap">Platform Fee</th>
                  <th className="p-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider font-mono whitespace-nowrap">Status</th>
                  <th className="p-4 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider font-mono text-right whitespace-nowrap">Date</th>
                  <th className="p-4 pr-6 text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider font-mono text-right whitespace-nowrap">Actions</th>
                </tr>
              </thead>
                ...
                {currentItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="p-4 font-display font-semibold text-[var(--text-primary)] whitespace-nowrap">₹{netAmount.toLocaleString('en-IN')}</td>
                      ...
                      <td className="p-4 pr-6 text-right whitespace-nowrap">
                        {statusLabel === 'PAID' && (
                          <button 
                            ...
                            className="text-xs text-[var(--violet)] hover:text-[var(--text-primary)] bg-[var(--violet)]/20 px-3 py-1.5 rounded-lg transition-colors border border-[#7C3AED]/30 flex items-center gap-1.5 ml-auto"
                          >
            ...
            <div className="p-6 overflow-y-auto">
              <form id="bank-form" onSubmit={handleBankSubmit} className="space-y-5">
                <div>
                  ...
                  <input required name="account_holder" defaultValue={bankDetails?.account_holder} type="text" className="w-full bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED]/50 transition-all font-medium" placeholder="As per bank records" />
                </div>
                
                <div>
                  <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-2">Account Number</label>
                  <input required name="account_number" defaultValue={bankDetails?.account_number} type="password" placeholder="Enter Account Number" className="w-full bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED]/50 transition-all font-mono tracking-wider" />
                </div>
                ...
                <div>
                  ...
                  <input required name="confirm_account_number" defaultValue={bankDetails?.account_number} type="text" placeholder="Re-enter Account Number" className="w-full bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED]/50 transition-all font-mono tracking-wider" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-2">IFSC Code</label>
                    <input required name="ifsc" defaultValue={bankDetails?.ifsc} type="text" placeholder="e.g. HDFC0001234" className="w-full bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED]/50 transition-all font-mono uppercase" />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider mb-2">Bank Name</label>
                    <input required name="bank_name" defaultValue={bankDetails?.bank_name} type="text" placeholder="e.g. HDFC Bank" className="w-full bg-[var(--bg-elevated)] border border-[var(--border-default)] rounded-xl px-4 py-3 text-[var(--text-primary)] focus:outline-none focus:border-[#7C3AED] focus:ring-1 focus:ring-[#7C3AED]/50 transition-all font-medium" />
                  </div>
                </div>
