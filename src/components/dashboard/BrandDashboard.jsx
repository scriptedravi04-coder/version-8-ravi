...
export default function BrandDashboard({ user }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-32 bg-[var(--bg-base)] min-h-screen w-full">
        <div className="flex flex-col items-center gap-4">
          <div className="w-8 h-8 rounded-full border-2 border-[#7C5CFF] border-t-transparent animate-spin"></div>
          <div className="text-[var(--text-tertiary)] text-sm font-mono tracking-widest uppercase">Loading Dashboard metrics...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)] py-10 px-4 sm:px-6 md:px-8 text-left w-full max-w-none relative overflow-x-hidden">
      ...
      <div className="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
        <div>
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-extrabold tracking-tight text-[var(--text-primary)] flex items-center gap-2">
        ...
        {showNotifPanel && (
          <>
            ...
            <motion.div 
              ...
              className="fixed top-0 right-0 w-full max-w-sm h-full bg-[var(--bg-card)] z-50 border-l border-border shadow-2xl flex flex-col"
            >
              <div className="flex items-center justify-between p-6 border-b border-[var(--border-default)]">
                ...
                <button 
                  onClick={() => setShowNotifPanel(false)} 
                  className="p-2 -mr-2 text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-elevated)] rounded-full transition-all cursor-pointer"
                >
                  <X size={20} />
                </button>
              </div>
              <div className="p-4 flex justify-end border-b border-[var(--border-default)] bg-white/[0.02]">
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {notifications.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-6 text-[var(--text-tertiary)]">
                  ...
                  notifications.map(n => (
                    <div 
                      key={n.id} 
                      onClick={() => setShowNotifPanel(false)} 
                      className={`p-4 rounded-xl hover:bg-[var(--bg-elevated)] transition-all duration-200 relative border text-left ${
                        !n.read ? 'border-[#7C5CFF]/30 bg-[#7C5CFF]/5 shadow-sm' : 'border-[var(--border-default)]'
                      }`}
                    >
                      {!n.read && (
                      ...
                      <h4 className="text-[var(--text-primary)] font-bold text-xs pr-6 leading-snug">{n.title}</h4>
                      <p className="text-[11px] text-[var(--text-secondary)] mt-1.5 leading-relaxed">{n.message}</p>
                      <div className="text-[9px] text-[var(--text-tertiary)] mt-3 font-semibold font-mono">JUST NOW</div>
                    </div>
                  ))
                )}
              </div>
